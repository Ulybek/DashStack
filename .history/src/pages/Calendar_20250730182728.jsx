...
export default function Calendar() {
    ...
    const handleAddEvent = () => {
        ...
        setEvents([newEventObj, ...events]); // Добавляем в начало списка
        setIsModalOpen(false); // Закрываем модалку
        setNewEvent({          // Сбрасываем форму
            title: '',
            location: '',
            date: '',
            time: '',
    ...
    // Клик по событию (открытие/закрытие popup)
    const handleEventClick = (clickInfo) => {
        if (selectedEvent && selectedEvent.id === clickInfo.event.id) {
            handleClosePopup(); // Если уже открыто — закрыть
        } else {
    ...
    // Инициализация Popper.js (позиционирование всплывающего окна)
    useEffect(() => {
    ...
    /** ------------------ Рендер ------------------ **/
    return (
        <div className='calendar-section page-main-section-style'>
            {/* Заголовок */}
            ...
            <div className='calendar-wrapper'>
                    ...
                    <ul className="event-list">
                        {events.map((event, idx) => (
                            <li key={event.id} className="event-item">
                                ...
                                {/* Разделитель (не у последнего элемента) */}
                ...
                <div className='calendar'>
                    <FullCalendar
                        ...
                        // Кастомный рендер события в календаре
                        eventContent={(arg) => {
                            const colorMap = {
                                '1': '#6C63FF',  // Фиолетовый
                                '2': '#FF6B6B',  // Красный
