...
export default function OrdersTable() {
    ...
    // Обработка выбора дат (поддержка мультивыбора)
    const handleDateClick = (date, event) => {
        if (event?.type === "contextmenu") {
            event.preventDefault(); // чтобы не открывалось меню
        }
        ...
        if (isMulti) {
            setSelectedDates((prev) =>
                exists ? prev.filter((d) => d.toDateString() !== dateStr) : [...prev, date]
            );
        } else {
    ...
    const applyFilter = () => {
        ...
        if (filters.type.length > 0) {
            filtered = filtered.filter((row) => filters.type.includes(row.type));
        }
