...
export default function OrdersTable({ darkMode, setDarkMode }) {
    ...
    useEffect(() => {
        if (openDropdown === "date" && dateButtonRef.current && datePopupRef.current) {
          datePopper.current = createPopper(dateButtonRef.current, datePopupRef.current, {
        ...
        } else if (datePopper.current) {
          datePopper.current.destroy();
          datePopper.current = null;
        }
      }, [openDropdown]);
