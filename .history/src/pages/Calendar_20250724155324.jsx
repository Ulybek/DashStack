...
export default function Calendar() {
    ...
    const handleEventClick = (clickInfo) => {
        if (selectedEvent && selectedEvent.id === clickInfo.event.id) {
            // Если клик по тому же событию — закрываем
            handleClosePopup();
        } else {
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            // Закрытие, если клик вне попапа и не по самому событию
            if (
                popperRef.current &&
    ...
    // Используем контейнер из FullCalendar
    const container = document.createElement('div');
    container.className = 'custom-event-fc';
    ...
    return { domNodes: [container] }; // Возвращаем как нативный DOM-элемент
}}
